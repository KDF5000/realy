...
func TestExecBindingHelper(t *testing.T) {
	if os.Getenv("RELAY_EXEC_HELPER") != "1" {
		return
	}
...
func TestExecProviderUsesStructuredStdinAndStdout(t *testing.T) {
	provider := binding.ExecProvider{Config: binding.Exec{Command: os.Args[0], Args: []string{"-test.run=TestExecBindingHelper"}, Env: map[string]string{"RELAY_EXEC_HELPER": "1"}}}
	output, err := provider.Invoke(context.Background(), relay.CapabilityRequest{Name: "issue.read", Version: "1", Resource: "MUL-42"})
	if err != nil {
		t.Fatal(err)
	}
