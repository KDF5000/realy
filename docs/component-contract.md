...
Realy 是可嵌入、也可通过 Server / Node 独立部署的分布式 Agent 执行基础组件。
...
自定义 Runtime 实现 `realy.Executor`，尊重 context 取消，在返回前停止并等待其所有
...
- realy-node 默认启用磁盘 outbox。事件经文件同步、原子发布和目录同步后才上报，
...
5. 下一阶段由真实业务接入继续验证公共接口；原生会话恢复等能力只在明确场景需要时加入。
